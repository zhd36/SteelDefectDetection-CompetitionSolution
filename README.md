...
## 分析
国赛主要突破点在数据上，对B,C进行分析可以发现由几种数据增强方法，包括对角线拼接，左右拼接，上下拼接，椒盐噪声，左上角四分之一加不规则矩形，亮暗度变化等。在C榜上加了增强之后miou普遍高于A，是因为存在数据泄露，B和C里数据的原图在A的训练和测试集中出现，为此，我们将评估方式改为对A增强的数据进行评估，来测试模型的泛化性。我们的方案有两大亮点：
1.原始的U2net的拟合能力很强，在我们使用深度可分离卷积降低参数量后，仍然有很强的拟合能力，且很难过拟合。
2.使用在线的数据增强，使训练数据尽可能的贴近C数据集的分布，在线的数据增强极大的扩大了等效数据量，搭配很难过拟合的模型，能够取得非常好的泛化性。
3.使用合理的评估方式，我们使用A在线增强的数据来进行评估模型的泛化性，使得我们能够快速迭代试错，在短时间内找到较好的策略训练一个较好的模型。
